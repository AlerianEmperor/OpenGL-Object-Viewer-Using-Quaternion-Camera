...
struct Model
{
	...
	void Read_Model(const string& filepath)
	{
		...
		while (file.getline(line, 1024))
		{
			...
			if (strncmp(t, "v", 1) == 0)
			{
				...
				/*else if (strncmp(t, "n", 1) == 0)
				{
					t += 1;
					int post_space = strspn(t, " \t");
					sscanf_s(t += post_space, "%f %f %f", &x, &y, &z);
		...
		float minX = min_vector.x;
		float minY = min_vector.y;
		float minZ = min_vector.z;

		float maxX = max_vector.x;
		float maxY = max_vector.y;
		float maxZ = max_vector.z;

		offset.x = (minX + maxX) * 0.5f;
		offset.y = (minY + maxY) * 0.5f;
		offset.z = (minZ + maxZ) * 0.5f;

		//float scaleX = maxX - minX;
		//float scaleY = maxY - minY;
		//float scaleZ = maxZ - minZ;

		float extend_x = maxX - minX;
		float extend_y = maxY - minY;
		float extend_z = maxZ - minZ;

		float max_extend = max(extend_x, extend_y);
		max_extend = max(max_extend, extend_z);

		float inv_max_extend = 1.2f / max_extend;


		for (int i = 0; i < vsize; ++i)
		{
			v[i] -= offset;
			v[i].x *= inv_max_extend;
			v[i].y *= inv_max_extend;
			v[i].z *= inv_max_extend;
		}
				...
				for (int j = 0; j < num_index; ++j)
				{
					for (int k = 0; k < 3; ++k)
					{
						...
						if (global_vertex_map.find(s) == global_vertex_map.end())
						{
							...
							vec3 vn0 = normalize(cross(v1 - v0, v2 - v0));//normal_map[ind_v].sum_normal;
				...
				for (int j = 0; j < num_index; ++j)
				{
					for (int k = 0; k < 3; ++k)
					{
						...
						if (global_vertex_map.find(ind_v) == global_vertex_map.end())
						{
							...
							vec3 vn0 = normalize(cross(v1 - v0, v2 - v0));//vn[ind_v];//normal_map[ind_v].sum_normal;
